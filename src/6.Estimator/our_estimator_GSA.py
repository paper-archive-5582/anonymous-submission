...
def success_rhs(beta, k, dim):
    ...
    term1 = math.sqrt(beta_f) / (2.0 * math.pi * math.e)
    term2 = (sqrt(t)) ** (-k_f / t * ln ((t/ beta_f)) )
    exponent = (-d + k_f + (beta_f * k_f / t))
    term3 = delta_beta ** exponent
...
def find_min_beta(n, logq, sigma,
    ...
    if sigma[0] > sigma[1]:
        w = math.sqrt(sigma[0] / sigma[1])
        sigma = math.sqrt(sigma[0])
    ...
    for beta in range(int(start), int(beta_max) + 1):
        ...
        m = round(sqrt((n+1)*logq/ log(delta_0,2) ) - n-1)
        dim = int(n + m + 1)
        k = find_k_for_time_match(beta, dim, k_max=k_max)
        # Using Bai-Galbraith embedding, the determinant of lattice is q^{m/d}w^{(n+1)/d}
        rhs = success_rhs(beta, k, dim)*2**(logq *m /dim + math.log2(w)*(n+1)/dim)
        lhs = lhs_for_secret_dist(s_dist, n, m, sigma, dim, sigma_s)
        if lhs < rhs:
            return {
              "beta": beta,
                "k": k,
...
def find_min_beta_bdd(n, logq, sigma,
    ...
    for beta in range(int(start), int(beta_max) + 1):
        ...
        k = find_k_for_time_match(beta, dim, k_max=k_max)
        # Using Bai-Galbraith embedding, the determinant of lattice is q^{m/d}w^{(n+1)/d}
        rhs = success_rhs_bdd(beta, k, dim)*2**(logq *m /dim + math.log2(w)*(n+1)/dim)
        if sigma < rhs:
            return {
                "beta": beta,
                "k": k,
...
def find_min_beta_usvp(n, logq, sigma,
    ...
    for beta in range(int(start), int(beta_max) + 1):
        ...
        k = find_k_for_time_match(beta, dim, k_max=k_max)
        # Using Bai-Galbraith embedding, the determinant of lattice is q^{m/d}w^{(n+1)/d}
        rhs = success_rhs_usvp(beta, k, dim)*2**(logq *m /dim + math.log2(w)*(n+1)/dim)
        if sigma < rhs:
            return {
                "beta": beta,
                "k": k,
